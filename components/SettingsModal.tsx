    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden transform transition-all">
          ...
          <div className="flex flex-col items-center">
             <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
               {formData.avatar ? (
                 ...
                 <div className="w-24 h-24 rounded-full bg-blue-600 dark:bg-slate-800 flex items-center justify-center text-white text-3xl font-bold shadow-lg">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input 
                ...
                className="w-full mt-1 bg-slate-100 dark:bg-slate-800 border-none rounded-lg px-4 py-2.5 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 dark:focus:ring-slate-600 outline-none"
              />
            </div>
            <div>
              ...
              {(['light', 'dark', 'default'] as Theme[]).map((theme) => (
                <button
                  ...
                  className={`px-3 py-2 rounded-lg text-sm font-medium border transition-all ${
                    currentTheme === theme
                      ? 'bg-blue-600 dark:bg-slate-700 text-white border-blue-600 dark:border-slate-700 shadow-md'
                      : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700 hover:border-blue-400 dark:hover:border-slate-500'
                  }`}
                >
        ...
        <div className="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 flex justify-between border-t border-slate-200 dark:border-slate-800">
           ...
           <button 
             onClick={handleSave}
             className="bg-blue-600 dark:bg-slate-900 hover:bg-blue-500 dark:hover:bg-slate-800 text-white text-sm font-semibold px-6 py-2 rounded-xl shadow-lg shadow-blue-500/30 dark:shadow-slate-900/50 transition-all hover:scale-105"
           >
             Save Changes
           </button>
        </div>
